...
///
/// 0. Run preparatory lowering passes, then rewrite pod-typed `llzk.nondet` ops into `pod.new`
///    so later stages only need to reason about POD storage through POD dialect operations.
///
...
struct RecordChain {
  ...
  /// Return the printable component name at `componentIndex`.
  StringRef getComponentName(size_t componentIndex) const {
    assert(componentIndex < nameList.size() && "component index must be in range");
    bool syntheticComponent = syntheticShapeCarrier && componentIndex + 1 == nameList.size();
    return syntheticComponent ? StringRef("shape") : nameList[componentIndex].getValue();
  }

  /// Return a copy of this chain with `prefix` inserted ahead of the current record names.
  RecordChain withPrefix(ArrayRef<StringAttr> prefix) const {
    SmallVector<StringAttr> fullChain(prefix.begin(), prefix.end());
    llvm::append_range(fullChain, nameList);
    return RecordChain(fullChain, syntheticShapeCarrier);
  }

  /// Build a flattened member name like `member_outer_inner_leaf`.
  StringAttr getFlattenedMemberName(MLIRContext *ctx, StringAttr memberName) const {
    std::string flatName;
    llvm::raw_string_ostream os(flatName);
    os << memberName.getValue();
    for (size_t i = 0; i < nameList.size(); ++i) {
      os << '_' << getComponentName(i);
    }
...
genArrayWrite(OpBuilder &bldr, Location loc, Value arrayRef, ArrayAttr index, Value value) {
  ...
  return genArrayWrite(bldr, loc, arrayRef, indices, value);
}
...
/// Prepare the module by resolving inferred type variables, lowering bool quantifiers, and
/// replacing `llzk.nondet` pod allocation ops with `pod.new`.
static LogicalResult step0(ModuleOp modOp) {
  MLIRContext *ctx = modOp.getContext();

  PassManager prepPM(ctx);
  prepPM.addPass(polymorphic::createTypeVarInferencePass());
  prepPM.addPass(boolean::createLowerBoolQuantifiersPass());
  if (failed(prepPM.run(modOp))) {
    return failure();
  }
    ...
    if (isNullOrEmpty(numDimsPerMap)) {
      if (adaptor.getElements().empty()) {
        ...
        if (needsPodArrayShapeCarrier(arrTy)) {
          ...
          preserveDiscardableAttrs(op, shapeCarrier.getDefiningOp());
          replacements.push_back(shapeCarrier);
        }
        rewriter.replaceOpWithMultiple(op, {ValueRange(replacements)});
        return success();
      }
      ...
      for (auto [id, splitType] : llvm::zip_equal(splitIds, splitTypes)) {
        ...
        if (!splitArray) {
          ...
          preserveDiscardableAttrs(op, splitArray.getDefiningOp());
        }
...
public:
  ...
  LogicalResult matchAndRewrite(
    ...
    for (auto [id, splitType, splitArrRange] :
      ...
      preserveDiscardableAttrs(op, leafValue.getDefiningOp());
...
public:
  ...
  LogicalResult matchAndRewrite(
    ...
    if (resultSplitTypes.empty()) {
      ...
      Value replacement = castValueToTypeIfNeeded(
          rewriter, op.getLoc(), splitInputs.front(), getPodArrayShapeCarrierType(resultArrTy)
      );
      if (replacement != splitInputs.front()) {
        preserveDiscardableAttrs(op, replacement.getDefiningOp());
      }
...
public:
  ...
  LogicalResult matchAndRewrite(
    ...
    if (rhsTy && rhsNeedsShapeCheck && !lhsTy &&
        (adaptor.getLhs().empty() || adaptor.getLhs().size() == 1)) {
      Value lhsSource =
          adaptor.getLhs().empty() ? op.getLhs() : getSingleConvertedValue(adaptor.getLhs());
      lhsCompatibleConvertedValues =
          materializeCompatiblePodArrayConvertedValues(op.getLoc(), lhsSource, rhsTy, rewriter);
    }
    if (lhsTy && lhsNeedsShapeCheck && !rhsTy &&
        (adaptor.getRhs().empty() || adaptor.getRhs().size() == 1)) {
      Value rhsSource =
          adaptor.getRhs().empty() ? op.getRhs() : getSingleConvertedValue(adaptor.getRhs());
      rhsCompatibleConvertedValues =
          materializeCompatiblePodArrayConvertedValues(op.getLoc(), rhsSource, lhsTy, rewriter);
    }

    ArrayType shapeCheckTy = lhsTy ? lhsTy : rhsTy;
    bool needsShapeCheck = lhsTy ? lhsNeedsShapeCheck : rhsNeedsShapeCheck;
    if (lhsTy && rhsTy) {
      ...
      needsShapeCheck = lhsNeedsShapeCheck || rhsNeedsShapeCheck;
    }

    if (shapeCheckTy && needsShapeCheck) {
      Value lhsShapeSource = getPodArrayEqualityShapeSource(
          lhsTy ? lhsTy : rhsTy, op.getLhs(),
          lhsTy ? adaptor.getLhs() : ValueRange(lhsCompatibleConvertedValues), op.getLoc(), rewriter
      );
      Value rhsShapeSource = getPodArrayEqualityShapeSource(
          rhsTy ? rhsTy : lhsTy, op.getRhs(),
          rhsTy ? adaptor.getRhs() : ValueRange(rhsCompatibleConvertedValues), op.getLoc(), rewriter
      );
    ...
    SmallVector<Value> lhsLeaves;
    if (!lhsTy && rhsTy && !lhsCompatibleConvertedValues.empty()) {
      ValueRange materializedLeavesForLhs =
          getConvertedPodArrayLeafValues(rhsTy, lhsCompatibleConvertedValues);
      llvm::append_range(lhsLeaves, materializedLeavesForLhs);
    } else {
      lhsLeaves = collectCompatiblePodArrayEqualityLeaves(
          op.getLoc(), op.getLhs(), adaptor.getLhs(), lhsTy, rhsTy, rewriter, materializedLeaves
      );
    }

    SmallVector<Value> rhsLeaves;
    if (!rhsTy && lhsTy && !rhsCompatibleConvertedValues.empty()) {
      ValueRange materializedLeavesForRhs =
          getConvertedPodArrayLeafValues(lhsTy, rhsCompatibleConvertedValues);
      llvm::append_range(rhsLeaves, materializedLeavesForRhs);
    } else {
      rhsLeaves = collectCompatiblePodArrayEqualityLeaves(
...
public:
  ...
  matchAndRewrite(ArrayLengthOp op, OpAdaptor, ConversionPatternRewriter &rewriter) const override {
    ...
    preserveDiscardableAttrsExcept(
        op, rewriter.replaceOpWithNewOp<ArrayLengthOp>(op, shapeSource, op.getDim()),
        DEFERRED_POD_ARRAY_LENGTH_ATTR
    );
    return success();
  }
};
...
public:
  ...
  LogicalResult matchAndRewrite(
    ...
    if (needsPodArrayShapeCarrier(resultTy)) {
      Value shapeCarrier = materializeExtractedPodArrayShapeCarrier(
...
public:
  ...
  rewrite(CreateArrayOp op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const override {
    ...
    preserveDiscardableAttrs(op, rebuiltArray.getDefiningOp());
...
static Value tryResolveReadPodArrayShapeSource(
  ...
  auto tryGetVirtualShapeSource = [&](ReadPodOp sourceRead) -> Value {
    ...
    for (auto [id, splitType] : llvm::zip_equal(splitIds, splitTypes)) {
      ...
      auto valueIt = podLeafValues->find(id.withPrefix({sourceRead.getRecordNameAttr()}));
...
public:
  ...
  LogicalResult matchAndRewrite(scf::IfOp ifOp, PatternRewriter &rewriter) const override {
    ...
    scf::YieldOp elseYieldOp = elseBlock ? getYieldOp(*elseBlock) : nullptr;
    ...
    appendYield(
        rewriter, ifOp.getLoc(), newThenBlock, originalThenYields, slots, true, thenYieldOp
    );
    appendYield(
        rewriter, ifOp.getLoc(), newElseBlock, originalElseYields, slots, false, elseYieldOp
    );
