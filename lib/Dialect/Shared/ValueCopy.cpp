...
/// Return whether copying a value of `type` preserves the same SSA value.
static bool isIdentityPreservingCopy(Type type) {
  if (llvm::isa<IndexType, IntegerType>(type)) {
    return true;
  }

  // These LLZK dialects define immutable scalar values. Struct values are identity-bearing handles:
  // copying one preserves the identity of the referenced mutable component rather than duplicating
  // its member storage. Keep this list explicit so a newly added mutable type cannot silently
  // acquire alias semantics merely because it lacks the interface.
