...
- Felt arguments are valid template arguments for parameterized struct types, free-function calls, and `verif.include` contract inclusions. A fieldless concrete felt or integer value may be materialized in a required field; an explicitly fielded concrete or symbolic value must use the same field. A symbolic value without a fielded type cannot satisfy a fielded restriction.
